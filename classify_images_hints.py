...
for pet_image in filename:
    low_split_pet_image = pet_image.lower().split("_")
    pet_name = " "
    for word in low_split_pet_image:
        if word.isalpha():
            pet_name += word + " "
    pet_name = pet_name.strip()
    pet_label.append(pet_name)
        
    results_dic = dict(zip(filename,pet_label))
...
results_dic = get_pet_labels(filename)
#print(results_dic)
model = "vgg"
model_labels = list()
model_label = list()
...
for i in range(len(filename)):
    image_dirs = ("pet_images/" + filename[i])
    image_dir.append(image_dirs)
print(image_dir)
#def classify_images(image_dir, results_dic, model):
for imagedir in image_dir:
    classifier_label = classifier(imagedir, model)
    model_labels.append(classifier_label)
for label in model_labels:    
    model_label.append(label.lower().strip())
#ltruth = list() 
filenames = list()
pet_label_value = list()
result_dic = dict()
for key,value in results_dic.items():
    filenames.append(key)
    pet_label_value.append(value)  


for idx in range (0, len(filenames), 1):
    if filenames[idx] not in result_dic:
        result_dic[filenames[idx]] = [ pet_label_value[idx], model_label[idx] ]
    if pet_label_value[idx] in model_label[idx]:
        result_dic[filenames[idx]].append(1)
    else:
        result_dic[filenames[idx]].append(0)
print(result_dic) 
            
    
    
    #def classify_images(image_dir, results_dic, model):
     #   classifier_labels = classifier(image_dic, model)
